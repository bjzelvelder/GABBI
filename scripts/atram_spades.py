...
class SpadesAssembler(BaseAssembler):
    ...
    def spades(self):
        ...
        if self.file['paired_count']:
            cmd.append("-1 '{}'".format(self.file['paired_1']))
            cmd.append("-2 '{}'".format(self.file['paired_2']))

        if self.file['single_1_count']:
            cmd.append("--s 1 '{}'".format(self.file['single_1']))
        if self.file['single_2_count']:
            cmd.append("--s 2 '{}'".format(self.file['single_2']))
        if self.file['single_any_count']:
            cmd.append("--s '{}'".format(self.file['single_any']))
        return ' '.join(cmd)

    def post_assembly(self):
        """Copy the assembler output."""
        src = join(self.work_path(), 'scaffolds.fasta')
